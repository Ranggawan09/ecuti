...
<div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
    ...
    <div class="sm:flex sm:justify-between sm:items-center mb-8">
        <div class="mb-4 sm:mb-0">
            <h1 class="text-2xl md:text-3xl text-gray-800 dark:text-gray-100 font-bold">Dashboard Atasan Tidak Langsung</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Selamat datang, <strong>{{ auth()->user()->nama ?? '-' }}</strong> 👋</p>
        </div>
        <div>
            <a href="{{ route('atasan-tidak-langsung.approvals.index') }}"
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <div class="col-span-2 lg:col-span-1 bg-gradient-to-br from-indigo-500 to-indigo-700 rounded-xl p-5 text-white shadow-sm">
            <div class="flex items-center justify-between mb-3">
                <div class="text-sm font-medium opacity-80">Menunggu Persetujuan</div>
                <div class="w-9 h-9 bg-white/20 rounded-full flex items-center justify-center">
                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                </div>
            </div>
            <div class="text-3xl font-bold mb-1">{{ $stats['menunggu'] }} <span class="text-lg font-normal opacity-80">pengajuan</span></div>
            <div class="text-xs opacity-70">Sudah disetujui atasan langsung</div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl p-5 shadow-sm border border-gray-100 dark:border-gray-700/60">
            ...
            <div class="text-xs text-gray-500 dark:text-gray-400 mt-1">Bawahan tidak langsung</div>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div class="bg-white dark:bg-gray-800 shadow-sm rounded-xl p-5 flex flex-col items-center">
            ...
            <p class="text-xs text-gray-400 dark:text-gray-500 mb-4 self-start">Perbandingan keputusan pengajuan</p>
            <canvas id="approvalDonutChart" class="max-w-[200px] max-h-[200px]"></canvas>
            <div class="flex flex-col gap-2 mt-4 text-xs w-full">
                <div class="flex items-center gap-1.5"><span class="w-3 h-3 rounded-full bg-indigo-400 inline-block"></span> Menunggu ({{ $stats['menunggu'] }})</div>
        ...
        <div class="lg:col-span-2 bg-white dark:bg-gray-800 shadow-sm rounded-xl">
            <header class="px-5 py-4 border-b border-gray-100 dark:border-gray-700/60 flex items-center justify-between">
                <h2 class="font-semibold text-gray-800 dark:text-gray-100">Daftar Pegawai Bawahan</h2>
                <span class="text-xs bg-indigo-100 dark:bg-indigo-500/30 text-indigo-600 dark:text-indigo-400 rounded-full px-2 py-1">{{ $stats['total_pegawai'] }} pegawai</span>
            </header>
            <div class="p-4">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-700/60">
                            ...
                            <tr>
                                <td colspan="3" class="p-6 text-center text-gray-400 dark:text-gray-500">
                                    ...
                                    Belum ada pegawai bawahan terdaftar.
                                </td>
                            </tr>
                            @endforelse
                        </tbody>
    ...
    <!-- Pengajuan Menunggu Persetujuan ATL -->
    @if($pendingRequests->isNotEmpty())
    <div class="bg-white dark:bg-gray-800 shadow-sm rounded-xl mb-6">
        <header class="px-5 py-4 border-b border-gray-100 dark:border-gray-700/60 flex items-center justify-between">
            <h2 class="font-semibold text-gray-800 dark:text-gray-100">Pengajuan Menunggu Persetujuan Anda</h2>
            <a href="{{ route('atasan-tidak-langsung.approvals.index') }}" class="text-sm text-indigo-500 hover:text-indigo-600 font-medium">Proses semua →</a>
        </header>
        <div class="p-4">
            <div class="space-y-3">
                @foreach($pendingRequests as $lr)
                <div class="flex items-center gap-3 p-3 rounded-lg bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-100 dark:border-indigo-500/20">
                    ...
                    <a href="{{ route('atasan-tidak-langsung.approvals.show', $lr) }}"
                       class="text-xs font-medium px-3 py-1.5 rounded-lg bg-indigo-500 hover:bg-indigo-600 text-white transition-colors shrink-0">
                        Tinjau
                    </a>
                </div>
                @endforeach
    ...
    <div class="bg-white dark:bg-gray-800 shadow-sm rounded-xl">
        <header class="px-5 py-4 border-b border-gray-100 dark:border-gray-700/60 flex items-center justify-between">
            ...
            <a href="{{ route('atasan-tidak-langsung.leave-history.index') }}" class="text-sm text-indigo-500 hover:text-indigo-600 font-medium">Lihat semua →</a>
        </header>
        <div class="p-4">
